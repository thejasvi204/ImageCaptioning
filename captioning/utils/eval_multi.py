...
def eval_allspice(dataset, preds_n, model_id, split):
    ...
    out = {}
    for metric, score in list(cocoEvalAllSPICE.eval.items()):
        out['All'+metric] = score

    imgToEvalAllSPICE = cocoEvalAllSPICE.imgToEval
    # collect SPICE_sub_score
    for k in list(list(imgToEvalAllSPICE.values())[0]['SPICE'].keys()):
        if k != 'All':
            out['AllSPICE_'+k] = np.array([v['SPICE'][k]['f'] for v in list(imgToEvalAllSPICE.values())])
...
def eval_oracle(dataset, preds_n, model_id, split):
    ...
    for i in range(len(capsById[list(capsById.keys())[0]])):
        preds = [_[i] for _ in list(capsById.values())]
        ...
        for img_id in list(capsById.keys()):
            tmp = imgToEval[img_id]
            for k in list(tmp['SPICE'].keys()):
                if k != 'All':
    ...
    for img_id in list(capsById.keys()):
        ...
        for metric in list(capsById[img_id][0]['scores'].keys()):
    ...
    for metric in list(list(out['ImgToEval'].values())[0].keys()):
        if metric == 'captions':
            continue
        tmp = np.array([_[metric] for _ in list(out['ImgToEval'].values())])
        tmp = tmp[tmp!=-100]
...
def eval_div_stats(dataset, preds_n, model_id, split):
    ...
    for k, score in zip(list(range(4)), all_scrs.mean(axis=0).tolist()):
