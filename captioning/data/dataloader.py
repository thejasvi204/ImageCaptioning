...
class Dataset(data.Dataset):
    ...
    def collate_func(self, batch, split):
        ...
        fc_batch, att_batch, label_batch, gts, infos = \
            list(zip(*sorted(zip(fc_batch, att_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True)))
        data = {}
        ...
        # generate mask
        nonzeros = np.array(list([(x != 0).sum()+2 for x in data['labels']]))
        ...
        data = {k:torch.from_numpy(v) if type(v) is np.ndarray else v for k,v in list(data.items())} # Turn all ndarray to torch tensor
...
class DataLoader:
    ...
    def state_dict(self):
        ...
        return {split: loader.sampler.state_dict(get_prefetch_num(split)) \
                    for split, loader in list(self.loaders.items())}

    def load_state_dict(self, state_dict=None):
        if state_dict is None:
            return
        for split in list(self.loaders.keys()):
