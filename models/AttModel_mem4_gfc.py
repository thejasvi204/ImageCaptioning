...
class AttModel_mem4_gfc(CaptionModel):
    def __init__(self, opt):
        super(AttModel_mem4_gfc, self).__init__()
        ...
        self.use_attr_info = opt.use_attr_info
        ...
        self.rela_dict_len = opt.rela_dict_size
        self.rela_embed = nn.Embedding(self.rela_dict_len, self.rnn_size)
        self.gate_embed = nn.Embedding(self.rela_dict_len, 1)

        self.sbj_fc = nn.Linear(self.rnn_size, self.rnn_size, bias=False)
        self.obj_fc = nn.Linear(self.rnn_size, self.rnn_size, bias=False)
        self.attr_fc = nn.Linear(self.rnn_size, self.rnn_size, bias=False)

        self.gate_sbj_fc = nn.Linear(self.rnn_size, 1, bias=False)
        self.gate_obj_fc = nn.Linear(self.rnn_size, 1, bias=False)
        self.gate_attr_fc = nn.Linear(self.rnn_size, 1, bias=False)
    ...
    def graph_gfc(self, rela_data):
        ...
        # att_feats: [N_img*5, N_att_max, rnn_size]
        rela_obj_feat = self.obj_fc(att_feats)  # [N_img*5, N_att_max, rnn_size]
        rela_sbj_feat = self.sbj_fc(att_feats)  # [N_img*5, N_att_max, rnn_size]
        gate_rela_obj_feat = self.gate_obj_fc(att_feats)  # [N_img*5, N_att_max, 1]
        gate_rela_sbj_feat = self.gate_sbj_fc(att_feats)  # [N_img*5, N_att_max, 1]
        ...
        for img_id in range(int(N_img)):
            ...
            obj_index = rela_matrix[img_id * seq_per_img, :N_rela, 1]  # [N_rela]
            rela_ids = rela_matrix[img_id * seq_per_img, :N_rela, 2].int().cpu().numpy()  # [N_rela]

            for box_id in range(N_box):
                # box_id as object
                sbj_id_temp = (sbj_index == box_id).nonzero().int().cpu().numpy()  # [N_sbj]
                if len(sbj_id_temp) > 0:
                    sbj_id_temp = np.squeeze(sbj_id_temp, axis=1)
                    sbj_ids = rela_matrix[img_id * seq_per_img, sbj_id_temp, 1].int().cpu().numpy()  # [N_sbj]
                    sbj_ids_feats = rela_sbj_feat[img_id * seq_per_img, sbj_ids, :]  # [N_sbj, rnn_size]
                    gate_sbj_ids_feat = gate_rela_sbj_feat[img_id * seq_per_img, sbj_ids, :]  # [N_sbj,1]
                    sbj_bias = rela_bias[img_id * seq_per_img, sbj_id_temp, :]  # [N_sbj,rnn_size]
                    gate_sbj_bias = gate_bias[img_id * seq_per_img, sbj_id_temp, :]  # [N_sbj,1]
                    gate_value = torch.sigmoid(gate_sbj_ids_feat + gate_sbj_bias)  # [N_sbj,1]
                    sbj_rela_feats_temp = sbj_ids_feats + sbj_bias  # [N_sbj, rnn_size]
                    sbj_rela_feats_temp = F.relu(
                ...
                if len(obj_id_temp) > 0:
                    obj_id_temp = np.squeeze(obj_id_temp, axis=1)
                    obj_ids = rela_matrix[img_id * seq_per_img, obj_id_temp, 1].int().cpu().numpy()  # [N_obj]
                    obj_ids_feats = rela_obj_feat[img_id * seq_per_img, obj_ids, :]  # [N_obj, rnn_size]
                    gate_obj_ids_feat = gate_rela_obj_feat[img_id * seq_per_img, obj_ids, :]  # [N_obj,1]
                    obj_bias = rela_bias[img_id * seq_per_img, obj_id_temp, :]  # [N_obj,rnn_size]
                    gate_obj_bias = gate_bias[img_id * seq_per_img, obj_id_temp, :]  # [N_obj,1]
                    gate_value = torch.sigmoid(gate_obj_ids_feat + gate_obj_bias)  # [N_obj,1]
                    obj_rela_feats_temp = obj_ids_feats + obj_bias  # [N_obj, rnn_size]
                    obj_rela_feats_temp = F.relu(
                ...
                if sbj_len != 0 or obj_len != 0:
                    rela_feats[box_id, :] = sbj_rela_feats_temp + obj_rela_feats_temp
                    box_use[box_id] = 1

            index = np.where(box_use == 1)[0]
            index_len = len(index)
            rela_masks_new[img_id * seq_per_img: (img_id + 1) * seq_per_img, :index_len] = 1
            rela_feats_new[img_id * seq_per_img: (img_id + 1) * seq_per_img, :index_len, :] = rela_feats[index, :]

        if self.use_attr_info:
            attr_obj_feats = self.attr_fc(att_feats)  # [N_img*5, N_att_max, rnn_size]
            ...
            for img_id in range(int(N_img)):
                ...
                N_attr = int(N_attr)
                attr_obj_ids = attr_matrix[img_id * seq_per_img, :N_attr, 0]  # [N_attr,1]
                attr_labels = attr_matrix[img_id * seq_per_img, :N_attr, 1:]  # [N_attr, N_attr_each_max]
                attr_bias = self.rela_embed(attr_labels.long())  # [N_attr, N_attr_each_max, rnn_size]
                gate_attr_bias = self.gate_embed(attr_labels.long())  # [N_attr, N_attr_each_max, 1]
                attr_feats = torch.zeros([N_attr, self.rnn_size])  # [N_attr, rnn_size]

                for attr_id in range(N_attr):
                    box_index = int(attr_obj_ids[attr_id])
                    N_attr_each = torch.sum(attr_masks[img_id * seq_per_img, attr_id, 1:])
                    N_attr_each = int(N_attr_each)
                    gate_feat_temp = gate_obj_feats[img_id * seq_per_img, box_index, :]  # [1, 1]
                    gate_bias_temp = gate_attr_bias[attr_id, :N_attr_each]  # [N_attr_each,1]
                    gate_feat_temp = gate_feat_temp.expand_as(gate_bias_temp)  # [N_attr_each,1]
                    gate_value = torch.sigmoid(gate_feat_temp + gate_bias_temp)  # [N_attr_each,1]

                    attr_feat_temp = attr_obj_feats[img_id * seq_per_img, box_index, :]  # [1, rnn_size]
                    attr_bias_temp = attr_bias[attr_id, :N_attr_each, :]  # [N_attr_each, rnn_size]
                    attr_bias_temp = torch.mm(
                        torch.t(gate_value), attr_bias_temp)  # [1*N_attr_each] * [N_attr_each*rnn_size]-> [1*rnn_size]
                    attr_feat = F.relu(attr_feat_temp + attr_bias_temp)  # [1*rnn_size]
                    attr_feats[attr_id] = attr_feat

                attr_feats_new[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_attr, :] = attr_feats
                attr_masks_new[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_attr] = 1

        rela_data['rela_feats'] = rela_feats_new
        rela_data['rela_masks'] = rela_masks_new
        if self.use_attr_info:
            rela_data['attr_feats'] = attr_feats_new
            rela_data['attr_masks'] = attr_masks_new
        return rela_data
        ...
        for img_id in range(int(N_img)):
            if self.use_attr_info != 0:
                ...
                max(N_att_new_max,torch.sum(rela_masks[img_id * seq_per_img, :]) +
                    torch.sum(att_masks[img_id * seq_per_img, :]) + torch.sum(attr_masks[img_id * seq_per_img,:]))
            else:
                N_att_new_max = \
                    max(N_att_new_max, torch.sum(rela_masks[img_id * seq_per_img, :]) +
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, rela_data=None, ssg_data=None, use_rela =1, training_mode = 0):
        ...
        if training_mode == 2:
            if use_rela == 1:
                ...
                rela_data = self.graph_gfc(rela_data)
...
class LSTM_mem4_gfc(AttModel_mem4_gfc):
    def __init__(self, opt):
        super(LSTM_mem4_gfc, self).__init__(opt)
        self.num_layers = 2
